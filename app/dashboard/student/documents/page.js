    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">Upload your CV, personal statement, and other application materials. Max 10MB per file.</p>
        </div>
        ...
        <div className="space-y-4">
          {groupedFiles.map(group => (
            <div key={group.value} className="bg-white rounded-lg border border-gray-200">
              <div className="px-5 py-3 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-sm font-semibold text-gray-700">{group.label}</h3>
                <span className="text-xs text-gray-400">{group.files.length} file{group.files.length !== 1 ? 's' : ''}</span>
              </div>
              {group.files.length === 0 ? (
                <div className="px-5 py-4 text-sm text-gray-400">No {group.label.toLowerCase()} uploaded yet.</div>
              ) : (
