...
const PUB_TYPES = [
  ...
  { value: 'co_author',    label: 'Co-Author Publication',    points: '3 pts', color: 'bg-teal-100 text-teal-700' },
  { value: 'poster',       label: 'Poster Presentation',     points: '1.5 pts', color: 'bg-teal-100 text-teal-700' },
  { value: 'abstract',     label: 'Abstract',                points: '1 pt',  color: 'bg-stone-100 text-stone-700' },
]
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-3xl mx-auto px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-stone-900">Publications</h2>
            <p className="text-stone-500 mt-1">{publications.length} publication{publications.length !== 1 ? 's' : ''}</p>
          </div>
          {!showForm && (
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-4 mb-5">
          <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-2">Competitiveness Score Impact</p>
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-stone-200 p-5 mb-5 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-semibold text-stone-700">{editingId ? 'Edit Publication' : 'New Publication'}</h3>
              <button type="button" onClick={resetForm} className="text-sm text-stone-400 hover:text-stone-600">Cancel</button>
            </div>

            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Publication Type <span className="text-rose-500">*</span></label>
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Journal / Conference</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">Publication Date</label>
                <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">DOI / URL</label>
            ...
            <button type="submit" disabled={saving}
              className="w-full bg-teal-600 text-white py-2 rounded-xl text-sm font-medium hover:bg-teal-700 disabled:opacity-50">
        ...
        {publications.length === 0 ? (
          ...
          <div className="space-y-3">
            {publications.map(pub => {
              ...
              return (
                <div key={pub.id} className="bg-white rounded-2xl border border-stone-200 p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        {t && <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${t.color}`}>{t.label}</span>}
                      </div>
                      <p className="font-medium text-stone-900 text-sm">{pub.title}</p>
                    ...
                    <div className="flex gap-3 ml-4">
                      <button onClick={() => handleEdit(pub)} className="text-sm text-stone-500 hover:text-stone-900">Edit</button>
                      <button onClick={() => handleDelete(pub.id)} className="text-sm text-stone-400 hover:text-rose-500">Delete</button>
                    </div>
                  </div>
                </div>
              )
