    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-3xl mx-auto px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-stone-900">Away Rotations</h2>
        ...
        <div className="bg-teal-50 border border-teal-200 rounded-lg p-4 mb-5 text-sm text-teal-800">
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-stone-200 p-5 mb-5 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-semibold text-stone-700">{editingId ? 'Edit Rotation' : 'New Away Rotation'}</h3>
              <button type="button" onClick={resetForm} className="text-sm text-stone-400 hover:text-stone-600">Cancel</button>
            </div>

            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Program / Hospital <span className="text-rose-500">*</span></label>
        ...
        {rotations.length === 0 ? (
          ...
          <div className="space-y-3">
            {rotations.map(rot => (
              <div key={rot.id} className="bg-white rounded-2xl border border-stone-200 p-5">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <select
                        ...
                        className={`text-xs px-2 py-0.5 rounded-full border-0 cursor-pointer focus:outline-none ${STATUS_COLORS[rot.status] || 'bg-stone-100 text-stone-600'}`}
                      >
                        <option value="planned">Planned</option>
                        <option value="completed">Completed</option>
                        <option value="cancelled">Cancelled</option>
                      </select>
                      {rot.specialty && <span className="text-xs text-stone-400">{rot.specialty}</span>}
                    </div>
                    ...
                    {rot.notes && <p className="text-xs text-stone-500 mt-1 italic">{rot.notes}</p>}
                  </div>
                  <div className="flex gap-3 ml-4">
                    <button onClick={() => handleEdit(rot)} className="text-sm text-stone-500 hover:text-stone-900">Edit</button>
                    <button onClick={() => handleDelete(rot.id)} className="text-sm text-stone-400 hover:text-rose-500">Delete</button>
                  </div>
                </div>
              </div>
            ))}
