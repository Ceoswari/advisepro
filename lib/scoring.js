...
export function calculateCompetitiveness(student, activitiesCount = 0, publications = [], awayRotationsCount = 0) {
  ...
  // --- Publications (uncapped, accumulate per pub) ---
  // Each publication type adds points — no hard ceiling so quality work is fully rewarded
  ...
  const pubScore = Math.round(
    firstAuthor * 6 +
    coAuthor    * 3 +
    poster      * 1.5 +
    abstract    * 1
  )
  // Display cap just for the progress bar (doesn't affect actual score)
  breakdown.publications = { score: pubScore, displayMax: 20, label: 'Publications', uncapped: true }
  total += pubScore

  // --- Research count (0–10 pts) ---
  const research = student.research_count || 0
  let researchScore = 0
  if (research >= 6)      researchScore = 10
  else if (research >= 4) researchScore = 8
  else if (research >= 2) researchScore = 5
  else if (research >= 1) researchScore = 3
  breakdown.research = { score: researchScore, max: 10, label: 'Research Experiences' }
  total += researchScore
  ...
  // --- Away Rotations (binary: 5 pts if any, 0 if none) ---
